...
| 2 | 1:00 | Attack simulation | **Run attack** | "The brief asks for paths eliminated — we count them, controls treated as perfect. Our agent also scores the feasible routes and picks among the best five, a thousand seeded times. Watch one: phish HR, dump, the file share, the payroll credential, the database — every roll against the technique's success probability, noise filling toward the detection budget." Let 2–3 trials play. | replay: **9 of 12** trials reach the crown jewel · favourite route `phish › cred_dump › smb_lateral › creds_in_files › db_login` |
| 3 | 1:30 | **The verdict** | tick **Segment prod-db (deny all inbound)**, then **Run again** | "Segment the database. Industry metric: 8 paths → 0, minus 100%. Success drops 0.89 → 0.10. And it **severs Payroll API → core DB, a P1 flow** — plus backup and ops maintenance. Verdict: BLOCK. Confidence Medium, because the backup account's admin right on the database is inferred, not observed." | paths **8 → 0** · effort **+14%** · p **0.89 → 0.10** · BREAKS **F1 (P1), F2 (P2), F6 (P3)** · confidence **Medium (0.83)** · **BLOCK** · replay now **2 of 12** — the attacker fails at `db_login → prod-db`, roll ≥ p 0.05 |
...
| 4a | 2:40 | **Adopt** | **Adopt into twin** on the DEPLOY card | "Nothing changed until now. Adopt it: the twin gets a child, the parent is kept, and the numbers at the top move — that is the board's decision recorded, not a deployment." Point at the −0.64 and the *applied* chip; press undo to show the parent comes back. | p(success) **0.89 → 0.23**, −0.64 since last twin · chip **applied** · breadcrumb FinBank → MFA… |
